...
struct GpuContext {
    ...
    float *xs = nullptr; // [MAX_SEQ, H]

    /* Host scratch, sized in warm_up() so inference() does not allocate. */
    std::vector<std::vector<int>> prompts;   /* [MAX_REQS][MAX_N_PROMPT] */
    std::vector<int> h_generated;            /* [MAX_SEQ] */
};
