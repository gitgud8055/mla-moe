...
- H = hidden_size ; NH = num_attention_heads      # n_kv_heads := NH (MLA, không GQA)
...
- rms_eps = rms_norm_eps (thiếu -> 1e-5) : input_layernorm / post_attn_norm / norm cuối
- norm_topk = norm_topk_prob (thiếu -> false) ; routed_scaling = routed_scaling_factor (dsv2: 1.0, glm: 1.8)
- rope_theta (thiếu -> 10000) ; rope_scaling{factor, beta_fast, beta_slow, original_max_position_embeddings}
...
- kv_stride = (QKN + VHD) * KVL          # stride 1 head trong kv_b_proj (Phase 2 & Decode)
...
- max_seq_len = min(max_position_embeddings, 163840)     # 163840 = KV_CACHE_CAP
- rope_interleaved = (model_type == "glm4_moe_lite") ? 1 : 0     # 1 = glm, 0 = dsv2
- router_sigmoid   = (topk_method == "noaux_tc")    ? 1 : 0      # 1 = glm, 0 = dsv2 softmax
- inv_freq[QKR/2]  = build_rope_inv_freq()  # YaRN, xem cuối file; tính 1 lần lúc load, dùng chung mọi layer
...
- eos = tokenizer_eos_id (không có tokenizer -> -1) ; max_new = 16 (MODE gen: mặc định 256)
...
- layer l = 0..n_layers-1:
  ...
  - qall[p, q_dim] = project_q(xb):
    + case q_lora_rank > 0:
      ...
      s_q_a = rmsnorm(s_q_a, w_q_a_layernorm[l, q_lora_rank], mla_norm_eps)   # in-place
  ...
  #   head h   -> w_W_UK[l] + h*kv_stride, w_W_UV[l] + h*kv_stride

  ## Phase 3 — attention.  q = 0..L-1, h = 0..NH-1:
  - qnope[QKN] = qall[q, h, 0:QKN] ; qpe[QKR] = qall[q, h, QKN:QKN+QKR]
  - k = 0..L-1:                          # causal: k > q -> -inf (softmax coi = 0)
    kpe[QKR]     = s_kv_cache[l, k, KVL:kvd]
    scr[h, q, k] = k <= q ? (qnope · knope[h, k, 0:QKN] + qpe · kpe) * scale : -inf
  - wgt[h, q, 0:L] = softmax(copy(scr[h, q, 0:L]))   # softmax CẢ hàng L; -inf -> 0 nên wgt[k>q] = 0
  - ctx[h, VHD] = Σ_{k=0..q} wgt[h, q, k] * value[h, k, 0:VHD]
  # sau khi đủ NH head (vẫn trong vòng q):
...
- LM head (mặc định: CHỈ vị trí cuối; mode ppl/teacher/dump lặp cùng công thức cho mọi p —
  ppl/teacher mỗi lần 1 hàng vocab, dump giữ cả [L, VOC]):
...
- layer l = 0..n_layers-1:
  ...
  - score[0:kv_len] = softmax(score[0:kv_len])   # không có -inf; đủ kv_len phần tử
  - clat[KVL] = Σ_{k=0..pos} score[k] * s_kv_cache[l, k, 0:KVL]
  - ctx[h, VHD] = clat @ w_W_UV[l, h, VHD x KVL].T            # CÓ .T
  # W_UK/W_UV lấy theo head: + h*kv_stride, cùng layout head-interleaved như Phase 2
...
- case l >= first_k_dense (MoE):
  ...
  + router_sigmoid == 1 (glm):  s_moe_logits[e] = sigmoid(s_moe_logits[e]) = 1/(1+exp(-.))
...
# gọi được in-place (y == x); run.c CHỈ dùng in-place cho s_q_a.
# c_kv thì KHÔNG in-place: đọc comp -> ghi thẳng vào s_kv_cache.
