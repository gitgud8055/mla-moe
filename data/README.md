...
Greedy decoding of the target model itself — HF `transformers` 5.12.1, **fp32**,
...
The reference was regenerated at **fp32** (it was previously bf16). The reason is that
greedy decoding is a hard argmax, so **any** small numeric difference (dtype, kernel,
quantization) can make a completion diverge from some token onward — and a bf16
reference diverged from an fp32-faithful engine on most prompts. At fp32 the reference
is reproducible token-for-token by an fp32 engine, which makes it usable as a strict
reference as well as a fuzzy one.

Concretely, against the previous bf16 reference an independent fp32 CPU engine matched
only 3/8 sampled completions exactly and scored 96.6% teacher-forced top-1; against this
fp32 reference the same engine matches 8/8 exactly and scores 100% top-1. Note that this
also means an engine using bf16/fp8 weights or KV cache will still diverge from this
reference on many prompts — score those with fuzzy metrics (METEOR + BERTScore-F1)
